  ...
  try {
    ...
    if (userData?.positive_vector) {
      ...
      const { data: matchData, error: matchError } = await supabase.rpc('match_content', {
        ...
        match_threshold: 0.1, // Low threshold to get more candidates for ranking
        match_count: limit * 5,
    ...
    // Fetch engagement details for candidates
    ...
    const ranked = candidates.map((c: any) => {
      ...
      // b. Engagement Score
      // Normalize engagements (likes + 2*comments + 3*shares)
      const engagement = (post.likes_count || 0) + (post.comments_count || 0) * 2 + (post.shares_count || 0) * 3;
      const engagementScore = Math.min(1.0, engagement / 100); // Caps at 100 interaction points

      // c. Recency Score (Exponential decay)
      const hoursSincePosted = (Date.now() - new Date(post.created_at).getTime()) / (1000 * 60 * 60);
      const recencyScore = Math.exp(-hoursSincePosted / 48); // 48h half-life

      // d. Final Combined Score
      const finalScore = (0.6 * similarity) + (0.2 * engagementScore) + (0.2 * recencyScore);
